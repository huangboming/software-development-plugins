...
Canonical rules for user-story format, role specificity, Given/When/Then acceptance criteria, observability, and edge-case coverage live in `../../write-user-story/references/story-template.md` (the Quality check and observability rewording test sections) and `../../write-user-story/references/splitting-patterns.md` (grain calibration and anti-patterns). Apply those rules when writing inline stories in a PRD so the inline set and any later standalone backlog stories teach the same standards.

The PRD-specific addition: inline stories in a feature PRD are the starting draft. `write-user-story` formalizes and grain-checks them into standalone backlog files — it does not invent a parallel set. Keep inline stories crisp; depth belongs in the backlog files.
